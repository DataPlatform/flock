...
class Table(object):
    ...
    def __init__(self, table_name, flock_app):
        ...
        self.flock = flock_app
        self.full_name = self.flock.name + '.' + self.database_table_name
        self.ddl_filename = self.get_schema_filename('definition')
        self.data_root = os.path.abspath(
            '{self.flock.settings.DATA_DIRECTORY}/{self.flock.environment_name}/{self.name}'.format(**dict(self=self)))

        # Need to store mappings of field names when they are transformed from
        # incoming csv's

        self.logger = self.flock.logger
    ...
    def get_ddl(self, temporary=False):
        ...
        if temporary:
            ddl = re.sub("create table", "create temporary table",
                         ddl, re.IGNORECASE)
            temp_tablename = self.flock.uuid('tmp')
    ...
    def set_ddl(self, ddl, fieldmap=None):
        ...
        self.flock.set_metadata(key, fieldmap)
    ...
    def primary_keys(self):
        cursor = self.flock.execute(""" SELECT               
    ...
    def fieldnames(self):
        sql = """SELECT column_name 
            ...
            where table_name = '{0}'
            and table_schema = '{1}'; """.format(self.database_table_name, self.flock.name)
        self.logger.debug(sql)
        cursor = self.flock.execute(sql)
    ...
    def number_of_rows_in_db(self):
        if self.table_exists():
            return self.flock.selectone('select count(*) from {0}'.format(self.full_name))
        else:
            return 0
    ...
    def clean_database(self, deep=False):
        if not deep:
            ...
            sql = 'delete from {0}.flock where key = \'{1}\';'.format(
                self.flock.name, self.name)
        self.flock.execute(sql)
        return self.flock.execute('drop table {0}'.format(self.full_name))
        ...
        if primary_keys:
            ...
            for infile in infiles:
                ...
                try:
                    ...
                    self.flock.execute("delete from {full_name} \
                ...
                self.flock.execute("drop table {temp_name};".format(**kwargs))

                rows_after = self.flock.selectone(
            ...
            csv_import(infiles, self.full_name, self.flock.db,
...
class Transaction:
    ...
    def savepoint(self):
        "Sets a savepoint"
        id = self.flock.uuid('sp')
        self.flock.execute('SAVEPOINT {0};'.format(id))
        self.flock.logger.debug('Setting database savepoint {0}'.format(id))
        return id

    def return_to_savepoint(self, id):
        "Returns to the specified savepoint"
        self.flock.logger.warn(
            'Returning to database savepoint {0}'.format(id))
        self.flock.execute('ROLLBACK TO SAVEPOINT {0};'.format(id))
