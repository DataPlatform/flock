...
class Schema(FlockSchema):
    ...
    def download_new_data(self, procurements):
        ...
        if beginning < yesterday:
            ...
            self.logger.info("Last update was {0}, attempting to update slice filedata".format(
            ...
            self.logger.info("Last update was {0}, refusing to update slice filedata".format(
                last_update.isoformat()))
    ...
    def bootstrap(self):
        ...
        # Get some sample data (sample size = all the data to date, as it is
        # quite small)
    ...
    def is_healthy(self):
        ...
        table_is_nonzero = self.selectone(
            "select (SELECT count(*) FROM fdps.procurements) > 0;")
        table_has_recent_data = self.selectone(
            "select (SELECT count(*) FROM fdps.procurements where transactionInformation_lastModifiedDate >= current_date - 7) > 0;")
